...
using System.IO;
using static ConvertPDF;
using static SeleniumDriver;
...
public static class ImageComparison
{
    ...
    {
        string differencesFilename = string.Format("{0:yyyy-MM-ddTHH-mm-ss}-Differences.png", DateTime.Now);
        ...
        // We already have the base image and will also create a difference image (new image taken but with areas in pink where we expected the element(s))
        // Save a copy of the new image
        string actualImageFilename = string.Format("{0:yyyy-MM-ddTHH-mm-ss}-ActualImage.png", DateTime.Now);
    ...
    public static int GetDifference(string image1, string image2)
    {
        ...
        if (File.Exists(fileDirectory + image1) && File.Exists(fileDirectory + image2))
        {
